import type { SVGProps } from "react";

type IconProps = SVGProps<SVGSVGElement> & {
  secondaryfill?: string;
  strokewidth?: number;
  title?: string;
};

function EaseInOut({
  fill = "currentColor",
  secondaryfill,
  strokewidth = 1,
  width = "1em",
  height = "1em",
  title = "badge 13",
  ...props
}: IconProps) {
  secondaryfill = secondaryfill || fill;

  return (
    <svg
      height={height}
      width={width}
      {...props}
      viewBox="0 0 18 18"
      xmlns="http://www.w3.org/2000/svg"
    >
      <title>{title}</title>
      <g fill={fill}>
        <path
          d="M7.88497 13.4283C6.83276 14.7133 5.24104 15.5 2.75 15.5C2.33579 15.5 2 15.1642 2 14.75C2 14.3358 2.33579 14 2.75 14C4.88396 14 6.01099 13.3492 6.72441 12.478C7.48997 11.543 7.86189 10.2694 8.27827 8.79603C8.28524 8.77137 8.29222 8.74664 8.29922 8.72186C8.69006 7.33815 9.13257 5.77154 10.115 4.57172C11.1672 3.28671 12.759 2.5 15.25 2.5C15.6642 2.5 16 2.83579 16 3.25C16 3.66421 15.6642 4 15.25 4C13.116 4 11.989 4.65079 11.2756 5.52203C10.51 6.45696 10.1381 7.73062 9.72173 9.20397C9.71476 9.22863 9.70778 9.25336 9.70078 9.27814C9.30994 10.6619 8.86744 12.2285 7.88497 13.4283Z"
          fill={secondaryfill}
          fillOpacity="0.4"
          fillRule="evenodd"
        />
        <path
          d="M5.13824 3.75C5.13824 3.33579 5.47403 3 5.88824 3H8.25001C8.66422 3 9.00001 3.33579 9.00001 3.75C9.00001 4.16421 8.66422 4.5 8.25001 4.5H5.88824C5.47403 4.5 5.13824 4.16421 5.13824 3.75Z"
          fill={fill}
          fillRule="evenodd"
        />
        <path
          d="M1.5 3.75C1.5 2.36979 2.61979 1.25 4 1.25C5.38021 1.25 6.5 2.36979 6.5 3.75C6.5 5.13021 5.38021 6.25 4 6.25C2.61979 6.25 1.5 5.13021 1.5 3.75ZM4 2.75C3.44821 2.75 3 3.19821 3 3.75C3 4.30179 3.44821 4.75 4 4.75C4.55179 4.75 5 4.30179 5 3.75C5 3.19821 4.55179 2.75 4 2.75Z"
          fill={fill}
          fillRule="evenodd"
        />
        <path
          d="M9 14.25C9 13.8358 9.33579 13.5 9.75 13.5H12.0003C12.4145 13.5 12.7503 13.8358 12.7503 14.25C12.7503 14.6642 12.4145 15 12.0003 15H9.75C9.33579 15 9 14.6642 9 14.25Z"
          fill={fill}
          fillRule="evenodd"
        />
        <path
          d="M14 13.25C13.4482 13.25 13 13.6982 13 14.25C13 14.8018 13.4482 15.25 14 15.25C14.5518 15.25 15 14.8018 15 14.25C15 13.6982 14.5518 13.25 14 13.25ZM11.5 14.25C11.5 12.8698 12.6198 11.75 14 11.75C15.3802 11.75 16.5 12.8698 16.5 14.25C16.5 15.6302 15.3802 16.75 14 16.75C12.6198 16.75 11.5 15.6302 11.5 14.25Z"
          fill={fill}
          fillRule="evenodd"
        />
      </g>
    </svg>
  );
}

export default EaseInOut;
