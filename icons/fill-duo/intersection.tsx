import type { SVGProps } from "react";

type IconProps = SVGProps<SVGSVGElement> & {
  secondaryfill?: string;
  strokewidth?: number;
  title?: string;
};

function Intersection({
  fill = "currentColor",
  secondaryfill,
  strokewidth = 1,
  width = "1em",
  height = "1em",
  title = "badge 13",
  ...props
}: IconProps) {
  secondaryfill = secondaryfill || fill;

  return (
    <svg
      height={height}
      width={width}
      {...props}
      viewBox="0 0 18 18"
      xmlns="http://www.w3.org/2000/svg"
    >
      <title>{title}</title>
      <g fill={fill}>
        <path
          d="M15.25 9.75C15.664 9.75 16 9.414 16 9C16 8.586 15.664 8.25 15.25 8.25C14.836 8.25 14.5 8.586 14.5 9C14.5 9.414 14.836 9.75 15.25 9.75Z"
          fill={fill}
        />
        <path
          d="M12.125 9.75C12.539 9.75 12.875 9.414 12.875 9C12.875 8.586 12.539 8.25 12.125 8.25C11.711 8.25 11.375 8.586 11.375 9C11.375 9.414 11.711 9.75 12.125 9.75Z"
          fill={fill}
        />
        <path
          d="M5.875 9.75C6.289 9.75 6.625 9.414 6.625 9C6.625 8.586 6.289 8.25 5.875 8.25C5.461 8.25 5.125 8.586 5.125 9C5.125 9.414 5.461 9.75 5.875 9.75Z"
          fill={fill}
        />
        <path
          d="M2.75 9.75C3.164 9.75 3.5 9.414 3.5 9C3.5 8.586 3.164 8.25 2.75 8.25C2.336 8.25 2 8.586 2 9C2 9.414 2.336 9.75 2.75 9.75Z"
          fill={fill}
        />
        <path
          d="M9 3.5C9.414 3.5 9.75 3.164 9.75 2.75C9.75 2.336 9.414 2 9 2C8.586 2 8.25 2.336 8.25 2.75C8.25 3.164 8.586 3.5 9 3.5Z"
          fill={fill}
        />
        <path
          d="M9 6.625C9.414 6.625 9.75 6.289 9.75 5.875C9.75 5.461 9.414 5.125 9 5.125C8.586 5.125 8.25 5.461 8.25 5.875C8.25 6.289 8.586 6.625 9 6.625Z"
          fill={fill}
        />
        <path
          d="M9 9.75C9.414 9.75 9.75 9.414 9.75 9C9.75 8.586 9.414 8.25 9 8.25C8.586 8.25 8.25 8.586 8.25 9C8.25 9.414 8.586 9.75 9 9.75Z"
          fill={fill}
        />
        <path
          d="M9 12.875C9.414 12.875 9.75 12.539 9.75 12.125C9.75 11.711 9.414 11.375 9 11.375C8.586 11.375 8.25 11.711 8.25 12.125C8.25 12.539 8.586 12.875 9 12.875Z"
          fill={fill}
        />
        <path
          d="M9 16C9.414 16 9.75 15.664 9.75 15.25C9.75 14.836 9.414 14.5 9 14.5C8.586 14.5 8.25 14.836 8.25 15.25C8.25 15.664 8.586 16 9 16Z"
          fill={fill}
        />
        <path
          d="M5.75 1C6.16421 1 6.5 1.33579 6.5 1.75V5.75C6.5 6.16421 6.16421 6.5 5.75 6.5H1.75C1.33579 6.5 1 6.16421 1 5.75C1 5.33579 1.33579 5 1.75 5H5V1.75C5 1.33579 5.33579 1 5.75 1Z"
          fill={secondaryfill}
          fillOpacity="0.4"
          fillRule="evenodd"
        />
        <path
          d="M12.25 1C12.6642 1 13 1.33579 13 1.75V5H16.25C16.6642 5 17 5.33579 17 5.75C17 6.16421 16.6642 6.5 16.25 6.5H12.25C11.8358 6.5 11.5 6.16421 11.5 5.75V1.75C11.5 1.33579 11.8358 1 12.25 1Z"
          fill={secondaryfill}
          fillOpacity="0.4"
          fillRule="evenodd"
        />
        <path
          d="M11.5 12.25C11.5 11.8358 11.8358 11.5 12.25 11.5H16.25C16.6642 11.5 17 11.8358 17 12.25C17 12.6642 16.6642 13 16.25 13H13V16.25C13 16.6642 12.6642 17 12.25 17C11.8358 17 11.5 16.6642 11.5 16.25V12.25Z"
          fill={secondaryfill}
          fillOpacity="0.4"
          fillRule="evenodd"
        />
        <path
          d="M1 12.25C1 11.8358 1.33579 11.5 1.75 11.5H5.75C6.16421 11.5 6.5 11.8358 6.5 12.25V16.25C6.5 16.6642 6.16421 17 5.75 17C5.33579 17 5 16.6642 5 16.25V13H1.75C1.33579 13 1 12.6642 1 12.25Z"
          fill={secondaryfill}
          fillOpacity="0.4"
          fillRule="evenodd"
        />
      </g>
    </svg>
  );
}

export default Intersection;
