import type { SVGProps } from "react";

type IconProps = SVGProps<SVGSVGElement> & {
  secondaryfill?: string;
  strokewidth?: number;
  title?: string;
};

function WindowHeart({
  fill = "currentColor",
  secondaryfill,
  strokewidth = 1,
  width = "1em",
  height = "1em",
  title = "badge 13",
  ...props
}: IconProps) {
  secondaryfill = secondaryfill || fill;

  return (
    <svg
      height={height}
      width={width}
      {...props}
      viewBox="0 0 18 18"
      xmlns="http://www.w3.org/2000/svg"
    >
      <title>{title}</title>
      <g fill={fill}>
        <path
          d="M14.2501 2H3.75009C2.23349 2 1.00009 3.2334 1.00009 4.75V7H17.0001V4.75C17.0001 3.2334 15.7667 2 14.2501 2ZM3.50009 5.5C2.94779 5.5 2.50009 5.0522 2.50009 4.5C2.50009 3.9478 2.94779 3.5 3.50009 3.5C4.05239 3.5 4.50009 3.9478 4.50009 4.5C4.50009 5.0522 4.05229 5.5 3.50009 5.5ZM6.50009 5.5C5.94779 5.5 5.50009 5.0522 5.50009 4.5C5.50009 3.9478 5.94779 3.5 6.50009 3.5C7.05239 3.5 7.50009 3.9478 7.50009 4.5C7.50009 5.0522 7.05229 5.5 6.50009 5.5Z"
          fill={fill}
        />
        <path
          d="M8.49909 13.2627C8.49229 11.0664 10.2921 9.2637 12.5123 9.25C13.0348 9.2568 13.5333 9.35348 13.9952 9.53418C14.4537 9.35258 14.9439 9.2559 15.4469 9.25C15.9986 9.2534 16.5221 9.36665 17.0001 9.56445V7H1.00009V13.25C1.00009 14.7686 2.23139 16 3.75009 16H9.24419C8.80079 15.2256 8.49909 14.3164 8.49909 13.2627Z"
          fill={secondaryfill}
          opacity="0.4"
        />
        <path
          d="M13.9991 17.999C13.8072 17.999 13.6158 17.9551 13.4405 17.8652C13.44 17.8652 13.439 17.8653 13.4385 17.8643C12.6314 17.4502 9.99901 15.8954 9.99901 13.2626C9.99461 11.8866 11.1264 10.7587 12.5215 10.7499C13.064 10.7567 13.5767 10.9286 13.9981 11.2294C14.419 10.9276 14.9293 10.7558 15.4634 10.7499C16.8731 10.7587 18.0044 11.8866 17.999 13.2655C17.999 15.8954 15.3667 17.4501 14.56 17.8641C14.3842 17.9539 14.1914 17.999 13.9991 17.999ZM14.1085 16.5225C14.1124 16.5254 14.1168 16.5273 14.1207 16.5292C14.1163 16.5263 14.1124 16.5244 14.1085 16.5225Z"
          fill={fill}
        />
      </g>
    </svg>
  );
}

export default WindowHeart;
