import type { SVGProps } from "react";

type IconProps = SVGProps<SVGSVGElement> & {
  secondaryfill?: string;
  strokewidth?: number;
  title?: string;
};

function UserClock({
  fill = "currentColor",
  secondaryfill,
  strokewidth = 1,
  width = "1em",
  height = "1em",
  title = "badge 13",
  ...props
}: IconProps) {
  secondaryfill = secondaryfill || fill;

  return (
    <svg
      height={height}
      width={width}
      {...props}
      viewBox="0 0 18 18"
      xmlns="http://www.w3.org/2000/svg"
    >
      <title>{title}</title>
      <g fill={fill}>
        <path
          d="M14 9.99951C11.7939 9.99951 10 11.7939 10 13.9995C10 16.2051 11.7939 17.9995 14 17.9995C16.2061 17.9995 18 16.2051 18 13.9995C18 11.7939 16.2061 9.99951 14 9.99951ZM16.3125 14.9497C16.1934 15.2393 15.9141 15.4145 15.6191 15.4145C15.5234 15.4145 15.4277 15.3964 15.3339 15.3583L13.7148 14.6933C13.4336 14.5776 13.25 14.3037 13.25 13.9995V12.2495C13.25 11.8354 13.5859 11.4995 14 11.4995C14.4141 11.4995 14.75 11.8354 14.75 12.2495V13.4966L15.9043 13.9707C16.2871 14.1279 16.4707 14.5664 16.3125 14.9497Z"
          fill={fill}
        />
        <path
          d="M11.1061 9.32314C9.54246 10.2938 8.49993 12.0266 8.49993 13.9995C8.49993 15.1039 8.82662 16.1331 9.38854 16.9954C9.26034 16.9985 9.13079 17 8.99993 17C6.99277 17 5.29497 16.6354 4.01253 16.2313C2.74419 15.8317 2.05155 14.4186 2.60511 13.1674C3.69052 10.7157 6.14161 9 8.99993 9C9.73429 9 10.4418 9.11325 11.1061 9.32314Z"
          fill={secondaryfill}
          fillOpacity="0.4"
        />
        <path
          d="M9 7.50049C10.7952 7.50049 12.25 6.04543 12.25 4.25049C12.25 2.45554 10.7952 1.00049 9 1.00049C7.20482 1.00049 5.75 2.45554 5.75 4.25049C5.75 6.04543 7.20482 7.50049 9 7.50049Z"
          fill={secondaryfill}
          fillOpacity="0.4"
        />
      </g>
    </svg>
  );
}

export default UserClock;
