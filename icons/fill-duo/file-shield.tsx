import type { SVGProps } from "react";

type IconProps = SVGProps<SVGSVGElement> & {
  secondaryfill?: string;
  strokewidth?: number;
  title?: string;
};

function FileShield({
  fill = "currentColor",
  secondaryfill,
  strokewidth = 1,
  width = "1em",
  height = "1em",
  title = "badge 13",
  ...props
}: IconProps) {
  secondaryfill = secondaryfill || fill;

  return (
    <svg
      height={height}
      width={width}
      {...props}
      viewBox="0 0 18 18"
      xmlns="http://www.w3.org/2000/svg"
    >
      <title>{title}</title>
      <g fill={fill}>
        <path
          d="M17.5605 11.3175L14.8105 10.0675C14.6132 9.97773 14.3867 9.97773 14.1894 10.0675L11.4395 11.3175C11.1719 11.4386 11 11.7062 11 12.0001V14.9405C11 16.9903 13.9604 17.878 14.2979 17.9727C14.3638 17.9913 14.4322 18 14.5 18C14.5678 18 14.6362 17.9912 14.7021 17.9727C15.0395 17.878 18 16.9903 18 14.9405V12.0001C18 11.7062 17.8281 11.4386 17.5605 11.3175ZM16.5 14.9405C16.5 15.5489 15.3726 16.1759 14.5 16.4659V11.5743L16.5 12.4835V14.9405Z"
          fill={fill}
        />
        <path
          d="M15.487 5.427L11.572 1.512C11.2442 1.1841 10.7996 1 10.336 1H4.75C3.2312 1 2 2.2312 2 3.75V14.25C2 15.7688 3.2312 17 4.75 17H10.1481C9.76919 16.4476 9.5 15.7654 9.5 14.9405V12.0001C9.5 11.1204 10.0136 10.317 10.8196 9.95158L13.568 8.70226C14.1597 8.43319 14.8395 8.43289 15.4312 8.70196L16 8.9605V6.6655C16 6.2009 15.8155 5.7553 15.487 5.427Z"
          fill={secondaryfill}
          fillOpacity="0.4"
          fillRule="evenodd"
        />
        <path
          d="M5 6.75C5 6.33579 5.33579 6 5.75 6H7.75C8.16421 6 8.5 6.33579 8.5 6.75C8.5 7.16421 8.16421 7.5 7.75 7.5H5.75C5.33579 7.5 5 7.16421 5 6.75Z"
          fill={fill}
        />
        <path
          d="M5.75 9C5.33579 9 5 9.33579 5 9.75C5 10.1642 5.33579 10.5 5.75 10.5H9.25C9.66421 10.5 10 10.1642 10 9.75C10 9.33579 9.66421 9 9.25 9H5.75Z"
          fill={fill}
        />
        <path
          d="M15.8691 6.00095H12C11.45 6.00095 11 5.55095 11 5.00095V1.13098C11.212 1.21803 11.4068 1.34674 11.572 1.51197L15.487 5.42697C15.6527 5.59263 15.7818 5.78817 15.8691 6.00095Z"
          fill={fill}
        />
      </g>
    </svg>
  );
}

export default FileShield;
