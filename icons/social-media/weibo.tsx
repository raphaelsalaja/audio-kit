import type { SVGProps } from "react";

type IconProps = SVGProps<SVGSVGElement> & {
  secondaryfill?: string;
  strokewidth?: number;
  title?: string;
};

function Weibo({
  fill = "currentColor",
  secondaryfill,
  strokewidth = 1,
  width = "1em",
  height = "1em",
  title = "badge 13",
  ...props
}: IconProps) {
  secondaryfill = secondaryfill || fill;

  return (
    <svg
      height={height}
      width={width}
      {...props}
      viewBox="0 0 32 32"
      xmlns="http://www.w3.org/2000/svg"
    >
      <title>{title}</title>
      <g fill={fill}>
        <path d="M13.443,27.064c-5.297,.522-9.87-1.872-10.215-5.35-.344-3.478,3.673-6.721,8.97-7.244,5.298-.524,9.871,1.871,10.215,5.347,.343,3.479-3.672,6.724-8.97,7.247m10.595-11.546c-.451-.135-.76-.227-.524-.818,.511-1.286,.564-2.395,.01-3.186-1.039-1.484-3.881-1.404-7.139-.04,0-.002-1.023,.448-.761-.364,.501-1.611,.426-2.96-.354-3.739-1.767-1.769-6.468,.067-10.499,4.096C1.753,14.486,0,17.685,0,20.452,0,25.744,6.786,28.961,13.425,28.961c8.703,0,14.492-5.057,14.492-9.071,0-2.425-2.043-3.802-3.879-4.372" />
        <path d="M29.818,5.835c-2.101-2.33-5.202-3.218-8.063-2.609h0c-.661,.141-1.084,.793-.942,1.454,.141,.661,.792,1.084,1.454,.942,2.036-.432,4.238,.201,5.733,1.855,1.493,1.655,1.898,3.913,1.259,5.891h0c-.209,.646,.144,1.336,.79,1.544,.643,.208,1.334-.144,1.542-.787v-.005c.897-2.785,.33-5.958-1.772-8.285" />
        <path d="M26.59,8.747c-1.023-1.134-2.533-1.566-3.927-1.269-.57,.121-.933,.682-.811,1.253,.122,.568,.682,.933,1.251,.809h0c.681-.144,1.42,.067,1.921,.62,.501,.555,.635,1.311,.42,1.975h0c-.179,.553,.124,1.148,.679,1.328,.555,.177,1.149-.125,1.328-.679,.437-1.357,.163-2.902-.862-4.037" />
        <path d="M13.735,20.765c-.185,.317-.595,.469-.916,.337-.316-.13-.415-.484-.235-.796,.185-.31,.579-.461,.893-.336,.32,.118,.435,.476,.258,.795m-1.688,2.167c-.512,.817-1.609,1.175-2.436,.798-.814-.371-1.055-1.321-.542-2.118,.506-.793,1.566-1.148,2.387-.803,.829,.354,1.095,1.297,.591,2.123m1.925-5.786c-2.521-.656-5.37,.6-6.465,2.822-1.115,2.265-.037,4.78,2.51,5.602,2.639,.851,5.748-.453,6.829-2.898,1.066-2.39-.265-4.851-2.874-5.526" />
      </g>
    </svg>
  );
}

export default Weibo;
