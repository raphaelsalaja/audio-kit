import type { SVGProps } from "react";

type IconProps = SVGProps<SVGSVGElement> & {
  secondaryfill?: string;
  strokewidth?: number;
  title?: string;
};

function HeartPulse({
  fill = "currentColor",
  secondaryfill,
  strokewidth = 1,
  width = "1em",
  height = "1em",
  title = "badge 13",
  ...props
}: IconProps) {
  secondaryfill = secondaryfill || fill;

  return (
    <svg
      height={height}
      width={width}
      {...props}
      viewBox="0 0 18 18"
      xmlns="http://www.w3.org/2000/svg"
    >
      <title>{title}</title>
      <g fill={fill}>
        <path
          d="M8.52901 15.222C8.82601 15.377 9.17301 15.377 9.47001 15.222C11.04 14.403 15.999 11.435 15.999 6.609C16.007 4.489 14.295 2.763 12.173 2.75C10.896 2.766 9.709 3.41 9 4.47C8.29 3.41 7.103 2.766 5.827 2.75C3.704 2.763 1.993 4.489 2.001 6.609C2.001 11.435 6.959 14.403 8.52901 15.222Z"
          fill={secondaryfill}
          fillOpacity="0.3"
          stroke="none"
        />
        <path
          d="M15.663 8.75C15.877 8.083 16 7.369 16 6.609C16.008 4.489 14.296 2.763 12.174 2.75C10.897 2.766 9.71 3.41 9.001 4.47C8.291 3.41 7.104 2.766 5.828 2.75C3.705 2.763 1.994 4.489 2.002 6.609C2.002 7.563 2.196 8.444 2.519 9.25"
          fill="none"
          stroke={fill}
          strokeLinecap="round"
          strokeLinejoin="round"
          strokeWidth={strokewidth}
        />
        <path
          d="M17.25 8.75H12L10.5 11.751L7.25 6.25L5.75 9.251H0.75"
          fill="none"
          stroke={fill}
          strokeLinecap="round"
          strokeLinejoin="round"
          strokeWidth={strokewidth}
        />
        <path
          d="M4.51349 12.2512C5.98729 13.7937 7.71499 14.7973 8.52999 15.222C8.82699 15.377 9.17399 15.377 9.47099 15.222C10.3714 14.7525 12.3843 13.5774 13.9393 11.75"
          fill="none"
          stroke={fill}
          strokeLinecap="round"
          strokeLinejoin="round"
          strokeWidth={strokewidth}
        />
      </g>
    </svg>
  );
}

export default HeartPulse;
